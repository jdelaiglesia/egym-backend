...
const updateDiscountCoupon = async (req, res) => {
  try {
    ...
    if (name === undefined || available === undefined) {
      ...
      const updateCoupon = await DiscountCoupon.findOneAndUpdate(
        { name: name },
        { available: available },
        { new: true }
      );
      if (!updateCoupon || updateCoupon.deleted) {
        ...
        res
          .status(200)
          .json({ message: "coupon updated correctly", updateCoupon });
      }
    }
  } catch (err) {
    res.status(400).json({ message: err.message });
...
const getDiscountCoupon = async (req, res) => {
  try {
    ...
    const searchCoupon = await DiscountCoupon.findOne({
      name: name,
      deleted: { $ne: true },
    });
    if (!searchCoupon) {
